...
def gradient_step(model: Layer, optimiser: Optimizer, loss_fn: Callable, x: paddle.Tensor, y: paddle.Tensor, **kwargs):
...
def batch_metrics(model: Layer, y_pred: paddle.Tensor, y: paddle.Tensor, metrics: List[Union[str, Callable]],
                  batch_logs: dict):
...
def fit(model: Layer, optimiser: Optimizer, loss_fn: Callable, epochs: int, dataloader: DataLoader,
    ...
    # Arguments
        ...
        dataloader: `paddle.DataLoader` instance to fit the model to
