...
# from paddle.nn.modules.loss import _Loss as Loss
import numpy as np
...
def matching_net_episode(model: Layer,
                         optimiser: Optimizer,
                         loss_fn: paddle.Tensor,  ###
                         x: paddle.Tensor,
                         y: paddle.Tensor,
                         n_shot: int,
                         k_way: int,
    ...
    # attention = (-distances).softmax(axis=1)
    attention = paddle.nn.functional.softmax(-distances , axis= 1)
    # attention = (-distances).Softmax(axis = 1)
    ...
    clipped_y_pred = y_pred.clip(EPSILON, 1 - EPSILON)
    ...
    if train:
        ...
        optimiser.grad_clip =paddle.nn.ClipGradByNorm(clip_norm=1.0)
        # Take gradient step
        optimiser.step()
...
def matching_net_predictions(attention: paddle.Tensor, n: int, k: int, q: int) -> paddle.Tensor:
    ...
    # Arguments
        attention: paddle.Tensor containing softmax over query-support distances.
