...
def pairwise_distances(x: paddle.Tensor,
                       y: paddle.Tensor,
                       matching_fn: str) -> paddle.Tensor:
    ...
    if matching_fn == 'l2':
        ...
        normalised_x = x / (x.pow(2).sum(axis=1, keepdim=True).sqrt() + EPSILON)
        normalised_y = y / (y.pow(2).sum(axis=1, keepdim=True).sqrt() + EPSILON)
...
def copy_weights(from_model: paddle.nn.Layer, to_model: paddle.nn.Layer):
    ...
    for m_from, m_to in zip(from_model.named_sublayers, to_model.named_sublayers):
    # for m_from, m_to in zip(from_model.modules(), to_model.modules()):
        is_linear = isinstance(m_to, paddle.nn.Linear)
        is_conv = isinstance(m_to, paddle.nn.Conv2D)
        is_bn = isinstance(m_to, paddle.nn.BatchNorm2D)
...
def autograd_graph(tensor: paddle.Tensor) -> Tuple[
            List[paddle.autograd.PyLayerContext], ###
            List[Tuple[paddle.autograd.PyLayerContext, paddle.autograd.PyLayerContext]] ###
        ]:
