...
class GlobalMaxPool1d(nn.Layer):
    ...
    """
    def forward(self, input):
        return nn.functional.max_pool1d(input, kernel_size=input.shape[2:]).reshape([-1, input.shape[1]])
...
class GlobalAvgPool2d(nn.Layer):
    ...
    """
    def forward(self, input):
        return nn.functional.avg_pool2d(input, kernel_size=input.shape()[2:]).reshape([-1, input.shape[1]])


def conv_block(in_channels: int, out_channels: int) -> nn.Layer:
    """Returns a Layer that performs 3x3 convolution, ReLu activation, 2x2 max pooling.
...
def functional_conv_block(x: paddle.Tensor, weights: paddle.Tensor, biases: paddle.Tensor,
                          bn_weights, bn_biases) -> paddle.Tensor:
...
# Models #
##########
def get_few_shot_encoder(num_input_channels=1) -> nn.Layer:
...
class FewShotClassifier(nn.Layer):
...
class MatchingNetwork(nn.Layer):
    def __init__(self, n: int, k: int, q: int, fce: bool, num_input_channels: int,
                 lstm_layers: int, lstm_input_size: int, unrolling_steps: int, device: paddle.device):
        ...
        if self.fce:
            ...
            self.g = paddle.cast(BidrectionalLSTM(lstm_input_size, lstm_layers) ,'float64')
            self.f = paddle.cast(AttentionLSTM(lstm_input_size, unrolling_steps=unrolling_steps) ,'float64')

    def forward(self, inputs):
        pass


class BidrectionalLSTM(nn.Layer):
...
class AttentionLSTM(nn.Layer):
    ...
    def forward(self, support, queries):
        ...
        h_hat = paddle.cast(paddle.zeros_like(queries) , 'float64')
        c = paddle.cast(paddle.zeros(batch_size, embedding_dim) , 'float64')
        ...
        for k in range(self.unrolling_steps):
            ...
            attentions = paddle.matmul(h, support.t())
            attentions = attentions.softmax(dim=1)

            # Calculate readouts from support set embeddings cf. equation (5)
            readout = paddle.matmul(attentions, support)
